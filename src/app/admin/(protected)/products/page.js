...
function StockBadge({ p }) {
  ...
  if (p.stockQty <= 0) return <span className="rounded-full bg-red-50 px-2 py-1 text-xs font-medium text-red-600">Hết hàng</span>;
  return (
...
// Compact card used on phones instead of the wide table. Details are hidden until "Xem thêm".
function MobileProductCard({ p, onDelete, deleting }) {
  const [open, setOpen] = useState(false);
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
      <div className="flex gap-3">
        <div
          className="flex h-16 w-16 flex-shrink-0 items-center justify-center overflow-hidden rounded-lg border border-slate-200"
      ...
      {open ? (
        <dl className="mt-3 space-y-1.5 rounded-lg bg-slate-50 p-3 text-xs">
          <div>
            ...
            <dd className="break-words text-slate-700">{p.nameVi}</dd>
          </div>
          <div>
            <dt className="text-slate-400">Hãng / Tỉ lệ</dt>
          ...
          <div>
            ...
            <dd className="break-all text-slate-700">{p.slug}</dd>
          </div>
        </dl>
      ) : null}

      <div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-2.5">
        <button type="button" onClick={() => setOpen((v) => !v)} className="text-xs font-semibold text-slate-500">
          {open ? "Thu gọn ▴" : "Xem thêm ▾"}
        </button>
        <div className="flex items-center gap-2">
          <Link href={`/admin/products/${p.id}`} className="rounded-lg border border-slate-300 px-4 py-1.5 text-xs font-semibold text-slate-700">
            Sửa
          </Link>
          <button
            type="button"
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Link
          ...
          className="whitespace-nowrap rounded-lg bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-800"
        >
          + Thêm sản phẩm
        </Link>
      </div>
      ...
      <div className="mt-6 space-y-3 md:hidden">
        {!loading && data.products.length === 0 ? (
          <p className="rounded-xl border border-slate-200 bg-white px-4 py-8 text-center text-sm text-slate-400">Chưa có sản phẩm nào.</p>
        ) : (
          data.products.map((p) => <MobileProductCard key={p.id} p={p} onDelete={handleDelete} deleting={deletingId === p.id} />)
        )}
      </div>
